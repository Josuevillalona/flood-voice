...
export default function CallLogFeed({ limit = 10 }: { limit?: number }) {
    ...
    useEffect(() => {
        const fetchLogs = async () => {
            const { data } = await supabase
                .from('call_logs')
                .select(`
                    id, started_at, ended_at, recording_url, transcript, summary, status, resident_id, tags, sentiment_score, key_topics,
        ...
        <div className="space-y-3">
            {logs.map((log) => (
                <Card key={log.id} className={cn(
                    ...
                    log.sentiment_score >= 8 ? "border-l-4 border-l-red-500" : ""
                )}>
                    <div
